...
static LogicalResult verifyInitializePipe(Operation *op, int8_t dirMask,
                                          int32_t slotSize, Type fifoType) {
  ...
  auto fifoTy = dyn_cast<pto::FifoType>(fifoType);
  if (!fifoTy)
    return op->emitOpError("result type must be !pto.fifo<...>");
  if (fifoTy.getDepth() == 0)
    return op->emitOpError("fifo depth must be positive");
  if (fifoTy.getNumBuffers() == 0)
    return op->emitOpError("fifo num_buffers must be positive");
  return success();
}
...
static LogicalResult verifyFifoTileType(Operation *op, Type fifoType,
                                        Type tileType, bool isPush) {
  auto fifoTy = dyn_cast<pto::FifoType>(fifoType);
  if (!fifoTy)
    return op->emitOpError("expects fifo operand type !pto.fifo<...>");

  Type expected = isPush ? fifoTy.getSrcTileType() : fifoTy.getDstTileType();
  if (tileType != expected) {
    return op->emitOpError(isPush ? "tile type must match fifo src tile type"
                                  : "tile type must match fifo dst tile type");
  }
  return success();
}
...
LogicalResult TPushOp::verify() {
  ...
  return verifyFifoTileType(getOperation(), getFifo().getType(),
...
void TPushOp::getEffects(
  ...
  addEffect(effects, &getFifoMutable(), MemoryEffects::Read::get());
  addEffect(effects, &getFifoMutable(), MemoryEffects::Write::get());
}
