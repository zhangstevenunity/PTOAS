module {
  func.func @matmul_tpush_tpop_print(
      ...
      %gm_slot_buffer: memref<16x16xf32, #pto.address_space<gm>>,
    ...
    %vec_mem = memref.alloc() : memref<16x16xf32, #pto.address_space<vec>>
    ...
    %vec_tile = pto.bind_tile %vec_mem, %c16, %c16 {
    ...
    } : memref<16x16xf32, #pto.address_space<vec>> -> memref<16x16xf32, #pto.address_space<vec>>

    %pipe = pto.initialize_pipe {dir_mask = 1, slot_size = 1024}
      (%gm_slot_buffer : memref<16x16xf32, #pto.address_space<gm>>,
       %c2v_consumer_buf : i32,
       %v2c_consumer_buf : i32)
      -> !pto.pipe<memref<16x16xf32, #pto.address_space<acc>>, memref<16x16xf32, #pto.address_space<vec>>>
    ...
    pto.section.cube {
      ...
      pto.tpush(%acc_tile, %pipe : memref<16x16xf32, #pto.address_space<acc>>, !pto.pipe<memref<16x16xf32, #pto.address_space<acc>>, memref<16x16xf32, #pto.address_space<vec>>>)
    }

    pto.section.vector {
      pto.tpop(%vec_tile, %pipe : memref<16x16xf32, #pto.address_space<vec>>, !pto.pipe<memref<16x16xf32, #pto.address_space<acc>>, memref<16x16xf32, #pto.address_space<vec>>>)
      pto.tmov ins(%vec_tile : memref<16x16xf32, #pto.address_space<vec>>) outs(%vec_print : !pto.tile_buf<loc=vec, dtype=f32, rows=16, cols=16, v_row=16, v_col=16, blayout=row_major, slayout=none_box, fractal=512, pad=0>)
